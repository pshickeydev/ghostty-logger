...
2. `run_session()` - pty lifecycle: `pty.fork()`, raw stdin, the `select`
   loop, child exit-code propagation.
3. `LogSession` - owns the log file and parser; `_guard()` contains parser
   faults so they degrade logging instead of killing the user's shell.
...
5. Helpers - `_open_log` (secure log file creation), `_winsize`,
   `_tag_prompt` (prompt recording indicator), `_timestamp`, etc.
